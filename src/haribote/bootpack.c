...
struct FIFO32 fifo_a = {0};
struct SHTCTL* shtctl = 0;
unsigned char* nihongo = 0, *chinese = 0;
...
void _HariMain(void) {
	...
	struct LANGUAGE language[2] = {
		{0, "nihongo.fnt", 16 * 256 + 32 * 94 * 47},
		{0, "chinese.fnt", 16 * 256 + 32 * 94 * 55}
	};
	extern char hankaku[4096];

	/* 英文键盘扫描码 */
	static char keytable0[0x80] = {		/* 无 Shift */
	...
	static char keytable1[0x80] = {		/* 有 Shift */
		0,   0,   '!', '@', '#', '$', '%', '^', '&',  '*', '(', ')', '_',  '+', 0x08, 0,
		'Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O',  'P', '{', '}', 0x0a, 0,   'A',  'S',
		'D', 'F', 'G', 'H', 'J', 'K', 'L', ':', '\"', '~', 0,   '|', 'Z',  'X', 'C',  'V',
		'B', 'N', 'M', '<', '>', '?', 0,   '*', 0,    ' ', 0,   0,   0,    0,   0,    0,
	...
	/* 载入 fnt */
	for (j = 0; j < 2; j++) {
		finfo = file_search(language[j].filename, (struct FILEINFO*)(ADR_DISKIMG + 0x002600), 224);
		if (finfo) {
			i = finfo->size;	/* 用 i 做中转，这是为了防止 finfo->size 的值被修改，因为 finfo->size 位于磁盘缓冲区中，如果被改掉将会影响 file_loadfile 函数读取的大小 */
			language[j].lang = file_loadfile2(finfo->clustno, &i);
		} else {
			language[j].lang = (unsigned char*)memman_alloc_4k(memman, language[j].filesize);
			for (i = 0; i < 16 * 256; i++) {
				language[j].lang[i] = hankaku[i];	/* 没有字库，半角部分直接复制英文字库 */
			}
			for (; i < language[j].filesize; i++) {
				language[j].lang[i] = 0xff;			/* 没有字库，全角以 0xff 填充 */
			}
		}
		if (j == 0) {
			nihongo = language[j].lang;
		} else {
			chinese = language[j].lang;
		}
	}
