...
/**
 * tek_getsize - 判断文件是否符合 tek 格式
 * @return: 如果是合法的 tek 格式则取得解压缩后的文件大小，如果不是合法的 tek 格式则返回 -1
 */
int tek_getsize(unsigned char* p) {
	...
	if (memcmp(p + 1, header, 15) == 0 && (*p == 0x83 || *p == 0x85 || *p == 0x89)) {
		/* memcmp 和 strncmp 差不多，这个函数忽略字符串中的 0 并一直比较到指定的字符数为止 */
		p += 16;
...
static int tek_lzrestore_tek5(int srcsiz, UCHAR *src, int outsiz, UCHAR *outbuf)
{
	...
	work = (int*)memman_alloc_4k(memman, wrksiz);
	if (work == NULL)
		return -1;
	flags = tek_decmain5(work, src, outsiz, outbuf, lc, pb, lp, flags);
	memman_free_4k(memman, (int)work, wrksiz);
	return flags;
}
