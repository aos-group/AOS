...
void cons_runcmd(char* cmdline, struct CONSOLE* cons, int* fat, unsigned int memtotal) {
	if (strcmp(cmdline, "mem") == 0 && cons->sht != 0) {	/* mem 命令 */
		...
		cmd_dir(cons);
	} else if (strncmp(cmdline, "type ", 5) == 0 && cons->sht != 0) {		/* type 命令 */
...
int cmd_app(struct CONSOLE* cons, int* fat, char* cmdline) {
	...
	if (finfo) {	/* 找到文件 */
		...
		if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
			...
			task->ds_base = (int)q;
			set_segmdesc(gdt + (task->sel >> 3) + 1000, finfo->size - 1, (int)p, AR_CODE32_ER_USER);
			set_segmdesc(gdt + (task->sel >> 3) + 2000, segsiz - 1, (int)q, AR_DATA32_RW_USER);
			for (i = 0; i < datsiz; i++) {
				q[esp + i] = p[dathrb + i];
			}
			start_app(0x1b, task->sel + (1000 << 3), esp, task->sel + (2000 << 3), &(task->tss.esp0));
...
int* hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax) {
	...
	if (edx == 1) {
		...
		if (eax == 0) {
			...
			i = 119318000 / eax;	/* PIT 时钟频率为 1.19318MHz */
			io_out8(0x43, 0xb6);
			io_out8(0x42, i & 0xff);
			io_out8(0x42, i >> 8);
			i = io_in8(0x61);
