...
void _HariMain(void) {
	...
	static char invstr0[32] = " abcd abcd abcd abcd abcd ";		/* len: 26，最左边和最右边留出空格原因：移动前不必将原图像刻意擦除，直接覆盖即可 */
...
next_group:		/* 下一关 */
	...
	while (1) {
		...
		if (keyflag[0 /* left */] != 0 && fx > 0) {
			fx--;
			putstr(win, winbuf, fx, 13, 6, "efg ");		/* 向左移动将最右边一格擦除 */
			keyflag[0 /* left */] = 0;
		} else if (keyflag[1 /* right */] != 0 && fx < 37) {
			putstr(win, winbuf, fx, 13, 6, " efg");		/* 向右移动将最左边一格擦除 */
			fx++;
			keyflag[1 /* right */] = 0;
