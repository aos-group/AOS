...
void HariMain(void) {
	char winbuf[256 * 112];		/* 窗口缓冲区 */
	char txtbuf[100 * 1024];	/* 文件内容缓冲区 */
	char s[30], *p, *r;
	int win, timer, i, j;
	int t = 120;	/* 乐曲速度 */
	int l = 192 /  4;	/* 音长 */
	...
	/* 音号与频率 (mHz) 的对照表 */
	/* 例如，04A 为 440Hz, 即 440000 */
	/* 第 16 八度的 A 为 1802240Hz, 即 1802240000 */
	/* 以下为第 16 八度的列表 (C~B) */

	static int tonetable[12] = {	/* 音 */
	...
	static int notetable[7] = {+9, +11, +0 /* C */, +2, +4, +5, +7};	/* 音符 */
...
file_error:
	...
	/* 准备窗口 */
	win = api_openwin(winbuf, 256, 112, -1, "mmlplay");		/* 30 * 6 */
	...
	r = txtbuf;
	i = 0;		/* 通常模式 */
	for (p = txtbuf; *p; p++) {		/* 为了方便处理，将注释和空白删去 */
		if (i == 0 && *p > ' ') {	/* 不是空格或换行符 */
			if (*p == '/') {
				if (p[1] == '*') {
					i = 1;
